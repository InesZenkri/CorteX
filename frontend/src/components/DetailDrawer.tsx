...
export function DetailDrawer({ item, onClose, onFinding, onCitation }: { item: GraphNode | GraphEdge; onClose: () => void; onFinding: (id: string) => void; onCitation: (citationId: string) => void }) {
  ...
  return <aside className="detail-drawer"><div className="drawer-top"><span className={`drawer-icon ${item.risk}`}><Icon size={18}/></span><button onClick={onClose} aria-label="Close details"><X size={18}/></button></div>
    ...
    <section><div className="section-title"><span>Linked findings</span><b>{item.findingIds.length}</b></div>{item.findingIds.length ? item.findingIds.map((id) => <button className="drawer-link" key={id} onClick={() => onFinding(id)}><div><strong>{id === 'f-shared-iban' ? 'Shared bank account' : 'Cash balance mismatch'}</strong><span>Open verified reasoning</span></div><ArrowRight size={15}/></button>) : <p className="empty-copy">No findings are linked to this entity.</p>}</section>
