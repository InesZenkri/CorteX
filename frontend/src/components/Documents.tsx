...
export function Documents({ folder, onUpload }: { folder?: UploadedFolder; onUpload: () => void }) {
  ...
  return <section className="page documents-page"><div className="page-heading"><div><p className="eyebrow">Uploaded evidence</p><h1>{folder.rootName}</h1><p className="lede">The directory structure below comes directly from the selected folder.</p></div><div className="document-summary"><div><strong>{folder.files.length} files</strong><span>{formatFileSize(folder.totalBytes)} total</span></div></div></div>
    <div className="document-workspace">
      <aside className="folder-browser"><div className="folder-browser-head"><h2>Folders</h2><span>{folder.files.length} files</span></div><FileTree items={folder.tree} onFileSelect={(id) => setPreview(folder.files.find((item) => item.id === id))}/></aside>
      <div className="document-content"><div className="document-toolbar"><label><Search size={15}/><input placeholder="Search uploaded files…"/></label><button><SlidersHorizontal size={14}/> Filter</button><div className="view-switch"><button className="active"><List size={14}/></button><button><Grid2X2 size={14}/></button></div></div>
        ...
        <div className="document-table"><div className="table-head uploaded-table-head"><span>File</span><span>Type</span><span>Location</span><span>Size</span><span>Modified</span></div>{folder.files.map((item) => <button className="table-row uploaded-table-row" key={item.id} onClick={() => setPreview(item)}><div><span className="file-type-badge">{item.extension.slice(0, 4).toUpperCase() || 'FILE'}</span><div><strong>{item.name}</strong><small>{item.relativePath}</small></div></div><span>{kindFor(item.extension)}</span><span className="path-cell">{item.relativePath.split('/').slice(0,-1).join(' / ') || 'Root'}</span><span>{formatFileSize(item.size)}</span><span>{new Intl.DateTimeFormat(undefined,{ dateStyle:'medium' }).format(item.modifiedAt)}</span></button>)}</div>
      </div>
    </div>
